import Head from 'next/head'
import Image from 'next/image'
import styles from '../styles/Home.module.css'

export default function Home() {
  return (
    <div className=''>
      <Head>
        <title>No Broker</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <div className='mt-24'>
        <div className='flex justify-center text-lg md:text-3xl  text-gray-500'>
          <h1>World's Largest NoBrokerage Property Site</h1>
        </div>
        <div className='flex justify-center mt-7 font-semibold  text-sm '>
          <h3 className='flex justify-center bg-orange-100  py-2 px-4 rounded-md'><img src="https://assets.nobroker.in/nb-new/public/nbCash.svg" />Payrent with Credit Card!</h3>
        </div>
        <div className='buyrent mt-8'>
          <ul className='flex justify-center '>
            <li className='border-[1px] border-gray-200 px-11 py-[9px]'>Buy</li>
            <li className='border-[1px] border-gray-200 px-10 py-[9px]'>Rent</li>
            <li className='border-[1px]  border-gray-200 px-4 py-[9px] '>Commercial</li>
          </ul>
          <div className="field flex justify-center">

            <select id="sites" className="dropdown4  py-3 px-5 border-[1px] font-semibold text-gray-500 bg-white ">
              <option value="">Mumbai</option>
              <option value="800 Sqft">800 Sqft</option>
              <option value="1000 Sqft">1000 Sqft</option>
              <option value="1200 Sqft">1200 Sqft</option>
            </select>
            <input className='px-48 border-[1px] ' type="text" id="demo" name="demo" placeholder="Search upto 3 localities" />
            <button className='px-14 py-3 border-1 bg-red-500 font-semibold text-white border-gray-200'><img />Search</button>
          </div>

          {/* ----------------------------------------------------------*/}


        </div>
        <div className='selectcontainer flex justify-center'>
          <div className='flex justify-start  w-[852px]  py-2 bg-gray-50 border-[1px]'>
            <div className='apartmentdropdown ml-[5px] px-1'>
              <select id="sites" className="dropdown  text-gray-500 text-xs py-[10px] px-10 border-[1px] bg-white ">
                <option value="" >Apartment Type</option>
                <option value="800 Sqft">800 Sqft</option>
                <option value="1000 Sqft">1000 Sqft</option>
                <option value="1200 Sqft">1200 Sqft</option>
              </select>
            </div>
            <div className='propertystatus px-1'>
              <select id="sites" className="dropdown text-gray-500 text-xs py-[10px] px-10 border-[1px] bg-white ">
                <option value="">Property Status</option>
                <option value="800 Sqft">800 Sqft</option>
                <option value="1000 Sqft">1000 Sqft</option>
                <option value="1200 Sqft">1200 Sqft</option>
              </select>
            </div>
            <div className='newbuilderprojects px-1'>
              <div className='text-gray-500 text-xs py-[10px] px-4 border-[1px] bg-white'><input className='mr-[7px]' type="checkbox" />New Builder Projects</div>
            </div>
          </div>
        </div>
        <div className='flex justify-center mt-14 font-semibold '>
          <div><hr className='w-24 mt-3 mr-3 border-gray-400' /></div>
          <div>Are you a Property Owner?</div>
          <div><hr className='w-24 mt-3 ml-3 border-gray-400' /></div>
        </div>

        <div className='freepropertybutton flex justify-center mt-7'>
          <button type="button" className='border-[1px] py-2 px-6 bg-green-600 text-white font-semibold'>Post Free Property ad</button>

        </div>
        <div className='items-center flex justify-center mt-4 text-sm py-3 text-white bg-indigo-900'>
          <img src="data:image/png;base64," />
          <h3 className='ml-2 mr-2'>Do you know how much <strong>loan</strong> you can get?Get maximum with <strong>NoBroker</strong></h3>
          <button className='border-[1px] bg-white text-black rounded-md px-1 py-1 text-xs'>Check Eligibility</button>
        </div>
        {/*------------------------Items div--------------------------------------------*/}

        <div>
          <div className='mt-10'>
            <ul className='flex justify-around text-sm text-center'>
              <li className=''><img className='w-[60px] h-[60px] mb-4 ml-2' src="https://tinyurl.com/yn59zx7v" />Home Loan</li>
              <li className='items-center'><img className='w-[60px] h-[60px] mb-4 ml-10' src="https://tinyurl.com/4z4jr4ut" />Property Legal Services</li>
              <li className='items-center'><img className='w-[60px] h-[60px] mb-4 ml-5' src="https://tinyurl.com/b3hb6md8" />Sale Agreement</li>
              <li className='items-center'><img className='w-[60px] h-[60px] mb-4 ml-7' src="https://tinyurl.com/25n9apy3" />New Builder Projects</li>
              <li className='items-center'><img className='w-[60px] h-[60px] mb-4' src="https://tinyurl.com/2r4kr522" />Interiors</li>
              <li className='items-center'><img className='w-[60px] h-[60px] mb-4 ml-6' src="https://tinyurl.com/yht5fve4" />NoBroker for NRI's</li>
            </ul>
          </div>
          <div className='whybox flex justify-center mt-10'>
            <div>
              <hr className='w-[600px] mt-3  border-gray-400 mr-48' />
            </div>
            <div className='w-[400px] ml-16'>
              <h1 className='text-3xl text-slate-500 '>Why Use NoBroker</h1>
            </div>
            <div className=''>
              <hr className='w-[600px] mt-3 ml-48 border-gray-400' />
            </div>
          </div>
          <div className='mt-16'>
            <ul className='flex  justify-between text-center ml-16 mr-16'>
              <li className='flex-column '><img className='ml-20' src="https://tinyurl.com/4hu549uy" /><h1 className='text-lg'>Avoid Brokers</h1><p>We directly connect you to verified<br /> owners to sav brokerage</p></li>
              <li className='flex-column '><img className='ml-16' src="https://tinyurl.com/5yhnfap2" /><h1 className='text-lg'>Free Listing</h1><p>Easy listing process.Also<br /> using WhatsApp</p></li>
              <li className='flex-column '><img className='ml-12' src="https://tinyurl.com/24yyxxm5" /><h1 className='text-lg'>Shortlist Without Visit</h1><p>Extensive Information<br />makes it easy</p></li>
              <li className='flex-column '><img className='ml-16' src="https://tinyurl.com/2s4e8vrj" /><h1 className='text-lg'>Rental Agreement</h1><p>Assitance in creating Rental<br /> agreement & Paper work</p></li>
            </ul>
          </div>
        </div>

        {/*-----------------------------NoBrokerAssist Plan-----------------------------------------------*/}

        <div className='bg-gray-100 mt-20 py-12'>
          <div className='whybox flex justify-center'>
            <div>
              <hr className='w-[400px] mt-3  border-gray-400 mr-48' />
            </div>
            <div className='w-[655px] ml-16'>
              <h1 className='text-3xl text-slate-500 '>NoBroker Business Assist Plan for Builders</h1>
            </div>
            <div className=''>
              <hr className='w-[400px] mt-3 ml-48 border-gray-400' />
            </div>
          </div>
          <div className='flex justify-center mt-20'>
            <img className='mr-32' src="data:image/png;base64," />
            <div>
              <h1 className='mt-10 text-md text-slate-500'>Get in touch with us to Sell or Rent Your Projects</h1>
              <button className='bg-red-500 mt-10 px-5 py-3 text-sm text-white'>Enquire Now</button>
              <div>
                <hr className='w-84 border-[1px] border-gray-300' />
              </div>
              <h3 className='text-sm'>For Builder Enquiries: +91 91080 50400</h3>
            </div>
          </div>
        </div>


        {/*------------------------------We Make difference----------------------------------*/}

        <div className='mt-12'>
          <div className='whybox flex justify-center'>
            <div>
              <hr className='w-[550px] mt-3  border-gray-400 mr-48' />
            </div>
            <div className='w-[500px] ml-16'>
              <h1 className='text-3xl text-slate-500 '>We Make A Difference</h1>
            </div>
            <div className=''>
              <hr className='w-[550px] mt-3 ml-48 border-gray-400' />
            </div>
          </div>
          <ul className='flex justify-evenly text-center mt-20'>
            <li className='border-red-600 text-3xl border-2 text-red-500 font-semibold rounded-full py-[62px] px-[20px]' >Rs.130 cr+</li>
            <li className='border-red-600 text-3xl border-2 text-red-500 font-semibold rounded-full py-[62px] px-[20px]' >30 Lakh+</li>
            <li className='border-red-600 text-3xl border-2 text-red-500 font-semibold rounded-full py-[62px] px-[20px]' >2  -Lakh+</li>
          </ul>
          <ul className='flex justify-evenly'>
            <li className='mt-5 ml-7 font-semibold text-lg text-gray-700'>Brokerage Saved Monthly</li>
            <li className='mt-5 font-semibold text-lg text-gray-700'>Customers Connected Monthly</li>
            <li className='mt-5 mr-10 font-semibold text-lg text-gray-700'>New Listings Monthly</li>
          </ul>
        </div>

        {/*----------------------------Our Customers Love Us---------------------------------------------------- */}

        <div className='items-center text-center bg-blue-400 mt-10 py-8' >
          <div className='text-3xl text-white'>
            <h1 >Our Customers Love us</h1>
          </div>

          <div className=' w-full flex justify-center mt-10 '>
            <div className='border-2 bg-gray-200 border-white py-[150px] px-[275px]' >
              <h1>Youtube</h1>
            </div>
          </div>
          <div>
            <div className='rows'></div>
            <div className='rows'></div>
            <div className='rows'></div>
          </div>

        </div>

        {/*---------------------------------------------------------------------------- */}

        <div className='flex justify-center mt-10'>
          <div>
            <img className='w-72' src="https://assets.nobroker.in/nb-new/public/Home/homeAppPromotion.png" />
          </div>
          <div className='ml-48'>
            <h1 className='text-3xl font-semibold text-red-500'>Find A New Home On The Go</h1>
            <h2 className='mt-10 text-xl text-gray-600'>Download our app</h2>
            <h3>Where convenience is at your fingertip</h3>
            <div className='flex mt-20'>
              <img src="https://tinyurl.com/ypfc3sm2" />
              <img src="https://tinyurl.com/mr2xe95e" className='ml-5 w-[165px]' />
            </div>
          </div>
        </div>



        {/*--------------------------------------------------------------------- */}
        
      
     
        
        <div className='mt-10 border-[1px]'>
          <div>
            <ul className='flex items-center bg-gray-100 justify-between  border-[1px] py-3 border-gray-300'>
              <li className='ml-10' >Flats for Sale</li>
              <li>Flats for Rent</li>
              <li>PG/Hostels</li>
              <li>Flatmates</li>
              <li>Miscellaneous</li>
              <li>Commercial</li>
              <li className='mr-10'>New Projects</li>
            </ul>
          </div>
          <div className='flex justify-evenly'>
            <div>
              <h1 className='ml-8 mt-2 text-xl'>Property for sail in Bangalore</h1>
              <ul className='ml-10'>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>

              </ul>
            </div>
            <div>
              <h1 className='ml-8 mt-2 text-xl'>Property for sail in Bangalore</h1>
              <ul className='ml-10'>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>

              </ul>
            </div>
            <div>
              <h1 className='ml-8 mt-2 text-xl'>Property for sail in Bangalore</h1>
              <ul className='ml-10'>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>

              </ul>
            </div>
            <div>
              <h1 className='ml-8 mt-2 text-xl'>Property for sail in Bangalore</h1>
              <ul className='ml-10'>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>

              </ul>
            </div>
            <div>
              <h1 className='ml-8 mt-2 text-xl'>Property for sail in Bangalore</h1>
              <ul className='ml-10'>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>
                <li>Flats for Sale in Koramangala</li>

              </ul>
            </div>
          </div>
        </div>
      


        {/*------------------------------------------------------------------------------ */}
      </div>

      {/* <div className='
             text-s  text-center
             bg-gray-500
             text-white
             fixed
             inset-x-0
             bottom-0
             
             '>
        <div className='flex justify-center '>
          <ul className='flex justify-evenly'>
            <li className='py-3 px-6 border-2'>Home</li>
            <li className='py-3 px-6 border-2'>Services</li>
            <li className='py-3 px-6 border-2'>Rewards</li>
            <li className='py-3 px-6 border-2'>Profile</li>
          </ul>
        </div>
      </div> */}
    </div>
  )
}
